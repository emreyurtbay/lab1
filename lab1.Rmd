...
The code above is creating a new variable called Elite, which sees if it is in the top 10 percent of rankings - this returns a TRUE or FALSE value. The second mutate actually recodes TRUE into "Yes" and FALSE into "No", so now the Elite variable only contains values that are the strings "Yes" or "No"
...
If we want to do linear modelling, we want to make sure that there exists a linear relationship between the predictor and response. It appears that there is indeed a linear relationship between acceptances and applications, which makes sense. We can likely use acceptances to predict 
...
```{r}
# add lm code here
 mymodel = lm(Apps ~ Accept, data=College)
summary(mymodel)
```
...
The plots do not look appropriate - we do not see normal residuals, and the residuals do not show homoscedastic errors. I would choose a different model.
